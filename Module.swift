            ...
            case.deadThursday:
                ...
                """).font(.system(size: inch(0.15)))
                    HStack(spacing: 0) {
                        ...
                        BasicTileView(appearance: Gift(coordinates: coordinates, fuelReward: 0, metalReward: 0, containedModule: StorageModule(tankId: nil), uuid: nil).appearance, accessibilitySettings: tank.playerInfo.accessibilitySettings)
                    }
...
class StorageModule: Module { override var type: ModuleType { .storage }
    override var view: any View {
        VStack(spacing: inch(0.25)) {
            ...
            Text("Fuel leakage is reduced by 50%.")
                .font(.system(size: inch(0.25)))
        }
    }
}
